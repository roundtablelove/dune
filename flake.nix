    ...
    in
    {
      ...
      packages =
        ...
        forAllSystems (
          ...
          # offline. Only the installables the Binaries workflow builds on
          # this platform are harvested: the trunk and devShell closures
          # (OxCaml, Rocq, ...) are not what those `nix build`s need, and
          # the musl and mingw cross closures do not build from Darwin.
          # mkSeed supports Linux and Darwin only and throws elsewhere, so
          # expose it only where it can be built -- flakeExposed also lists
          # freebsd, i686 and friends.
